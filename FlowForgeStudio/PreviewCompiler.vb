...
Namespace FlowForgeStudio
    Public NotInheritable Class PreviewCompiler
        ...
        Private Shared Function Compile(project As FlowProject, exe As String, runAfterBuild As Boolean) As String
            ...
            If result.Errors.HasErrors Then
                ...
                For Each err As CompilerError In result.Errors
                    If Not err.IsWarning Then output.AppendLine(String.Format("{0} — linha {1}, coluna {2}: {3} {4}", Path.GetFileName(err.FileName), err.Line, err.Column, err.ErrorNumber, err.ErrorText))
                Next
        ...
        Private Shared Function PropertyLoaderSource() As String
            ' IMPORTANTE:
            ' O código abaixo é VB.NET que será compilado junto com o aplicativo do usuário.
            ' Mantemos cada If/For/Select em linhas próprias. O compilador VB não aceita
            ' de forma confiável blocos estruturados complexos comprimidos em uma linha.
            Dim q As String = System.Convert.ToChar(34).ToString()
            Dim b As New StringBuilder()

            b.AppendLine("Imports System")
            b.AppendLine("Imports System.ComponentModel")
            b.AppendLine("Imports System.Collections.Generic")
            b.AppendLine("Imports System.Linq")
            b.AppendLine("Imports System.Text")
            b.AppendLine("Imports System.Windows.Forms")
            b.AppendLine("")
            b.AppendLine("Public Module PropertyLoader")

            b.AppendLine(" Private Function DecodeToken(value As String) As String")
            b.AppendLine("  Try")
            b.AppendLine("   Return Encoding.UTF8.GetString(Convert.FromBase64String(value))")
            b.AppendLine("  Catch")
            b.AppendLine("   Return " & q & q)
            b.AppendLine("  End Try")
            b.AppendLine(" End Function")

            b.AppendLine(" Private Function DecodeList(value As String) As String()")
            b.AppendLine("  If String.IsNullOrEmpty(value) Then")
            b.AppendLine("   Return New String() {}")
            b.AppendLine("  End If")
            b.AppendLine("  Return value.Split(" & q & ";" & q & "c).Select(Function(x) DecodeToken(x)).ToArray()")
            b.AppendLine(" End Function")

            b.AppendLine(" Public Sub Apply(target As Object, name As String, value As String)")
            b.AppendLine("  Try")

            ' ComboBox/ListBox/CheckedListBox.Items
            b.AppendLine("   If name = " & q & "__FF_ITEMS" & q & " Then")
            b.AppendLine("    Dim values As String() = DecodeList(value)")
            b.AppendLine("    If TypeOf target Is ComboBox Then")
            b.AppendLine("     Dim c As ComboBox = DirectCast(target, ComboBox)")
            b.AppendLine("     c.Items.Clear()")
            b.AppendLine("     c.Items.AddRange(values.Cast(Of Object)().ToArray())")
            b.AppendLine("    ElseIf TypeOf target Is CheckedListBox Then")
            b.AppendLine("     Dim c As CheckedListBox = DirectCast(target, CheckedListBox)")
            b.AppendLine("     c.Items.Clear()")
            b.AppendLine("     c.Items.AddRange(values.Cast(Of Object)().ToArray())")
            b.AppendLine("    ElseIf TypeOf target Is ListBox Then")
            b.AppendLine("     Dim c As ListBox = DirectCast(target, ListBox)")
            b.AppendLine("     c.Items.Clear()")
            b.AppendLine("     c.Items.AddRange(values.Cast(Of Object)().ToArray())")
            b.AppendLine("    End If")
            b.AppendLine("    Return")
            ...
            b.AppendLine("   If name = " & q & "__FF_COLUMNS" & q & " AndAlso TypeOf target Is DataGridView Then")
            b.AppendLine("    Dim grid As DataGridView = DirectCast(target, DataGridView)")
            b.AppendLine("    grid.Columns.Clear()")
            b.AppendLine("    If Not String.IsNullOrWhiteSpace(value) Then")
            b.AppendLine("     For Each row As String In value.Split(" & q & ";" & q & "c)")
            b.AppendLine("      Dim parts As String() = row.Split(" & q & "," & q & "c)")
            b.AppendLine("      If parts.Length >= 5 Then")
            b.AppendLine("       Dim col As DataGridViewColumn")
            b.AppendLine("       Select Case DecodeToken(parts(0))")
            b.AppendLine("        Case " & q & "DataGridViewCheckBoxColumn" & q)
            b.AppendLine("         col = New DataGridViewCheckBoxColumn()")
            b.AppendLine("        Case " & q & "DataGridViewButtonColumn" & q)
            b.AppendLine("         col = New DataGridViewButtonColumn()")
            b.AppendLine("        Case " & q & "DataGridViewComboBoxColumn" & q)
            b.AppendLine("         col = New DataGridViewComboBoxColumn()")
            b.AppendLine("        Case " & q & "DataGridViewImageColumn" & q)
            b.AppendLine("         col = New DataGridViewImageColumn()")
            b.AppendLine("        Case " & q & "DataGridViewLinkColumn" & q)
            b.AppendLine("         col = New DataGridViewLinkColumn()")
            b.AppendLine("        Case Else")
            b.AppendLine("         col = New DataGridViewTextBoxColumn()")
            b.AppendLine("       End Select")
            b.AppendLine("       col.Name = DecodeToken(parts(1))")
            b.AppendLine("       col.HeaderText = DecodeToken(parts(2))")
            b.AppendLine("       Dim width As Integer = 100")
            b.AppendLine("       Integer.TryParse(parts(3), width)")
            b.AppendLine("       col.Width = Math.Max(20, width)")
            b.AppendLine("       Dim isVisible As Boolean = True")
            b.AppendLine("       Boolean.TryParse(parts(4), isVisible)")
            b.AppendLine("       col.Visible = isVisible")
            b.AppendLine("       grid.Columns.Add(col)")
            b.AppendLine("      End If")
            b.AppendLine("     Next")
            b.AppendLine("    End If")
            b.AppendLine("    Return")
            ...
            ' TabControl.TabPages
            b.AppendLine("   If name = " & q & "__FF_TABPAGES" & q & " AndAlso TypeOf target Is TabControl Then")
            b.AppendLine("    Dim tabs As TabControl = DirectCast(target, TabControl)")
            b.AppendLine("    tabs.TabPages.Clear()")
            b.AppendLine("    If Not String.IsNullOrWhiteSpace(value) Then")
            b.AppendLine("     For Each row As String In value.Split(" & q & ";" & q & "c)")
            b.AppendLine("      Dim parts As String() = row.Split(" & q & "," & q & "c)")
            b.AppendLine("      If parts.Length >= 2 Then")
            b.AppendLine("       Dim page As New TabPage(DecodeToken(parts(1)))")
            b.AppendLine("       page.Name = DecodeToken(parts(0))")
            b.AppendLine("       tabs.TabPages.Add(page)")
            b.AppendLine("      End If")
            b.AppendLine("     Next")
            b.AppendLine("    End If")
            b.AppendLine("    Return")
            ...
            ' TreeView.Nodes
            b.AppendLine("   If name = " & q & "__FF_TREENODES" & q & " AndAlso TypeOf target Is TreeView Then")
            b.AppendLine("    Dim tree As TreeView = DirectCast(target, TreeView)")
            b.AppendLine("    tree.Nodes.Clear()")
            b.AppendLine("    Dim parents As New Dictionary(Of Integer, TreeNode)()")
            b.AppendLine("    If Not String.IsNullOrWhiteSpace(value) Then")
            b.AppendLine("     For Each row As String In value.Split(" & q & ";" & q & "c)")
            b.AppendLine("      Dim parts As String() = row.Split(" & q & "," & q & "c)")
            b.AppendLine("      If parts.Length >= 3 Then")
            b.AppendLine("       Dim depth As Integer = 0")
            b.AppendLine("       Integer.TryParse(parts(0), depth)")
            b.AppendLine("       Dim node As New TreeNode(DecodeToken(parts(2)))")
            b.AppendLine("       node.Name = DecodeToken(parts(1))")
            b.AppendLine("       If depth <= 0 OrElse Not parents.ContainsKey(depth - 1) Then")
            b.AppendLine("        tree.Nodes.Add(node)")
            b.AppendLine("       Else")
            b.AppendLine("        parents(depth - 1).Nodes.Add(node)")
            b.AppendLine("       End If")
            b.AppendLine("       parents(depth) = node")
            b.AppendLine("      End If")
            b.AppendLine("     Next")
            b.AppendLine("    End If")
            b.AppendLine("    Return")
            ...
            ' Propriedades normais continuam usando o TypeDescriptor.
            b.AppendLine("   Dim pd As PropertyDescriptor = TypeDescriptor.GetProperties(target)(name)")
            b.AppendLine("   If pd IsNot Nothing AndAlso Not pd.IsReadOnly AndAlso pd.Converter.CanConvertFrom(GetType(String)) Then")
            b.AppendLine("    pd.SetValue(target, pd.Converter.ConvertFromInvariantString(value))")
            b.AppendLine("   End If")

            b.AppendLine("  Catch")
            b.AppendLine("   ' Uma propriedade incompatível não deve impedir a abertura do formulário.")
            b.AppendLine("  End Try")
            b.AppendLine(" End Sub")
            b.AppendLine("End Module")

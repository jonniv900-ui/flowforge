...
Namespace FlowForgeStudio
    Public Class MainForm
        ...
        Private educationTools As ToolStrip
        ...
        Private ReadOnly educationTip As New ToolStripStatusLabel("Dica: selecione um componente para aprender sobre ele")
        ...
        Private beginnerMode As Boolean = True
        Private beginnerModeItem As ToolStripMenuItem
        Private ReadOnly advancedMenus As New List(Of ToolStripItem)()
        Private ReadOnly recoveryTimer As New System.Windows.Forms.Timer() With {.Interval = 60000}
        ...
        Public Sub New(Optional initialProjectPath As String = Nothing)
            Text = "FlowForge Education" : WindowState = FormWindowState.Maximized : MinimumSize = New Size(1100, 700)
            Try
                Dim appIconPath As String = Path.Combine(Application.StartupPath, "FlowForgeEducation.ico")
            ...
            ApplyEducationMode()
            AddHandler Shown, AddressOf MainFormShown
            AddHandler FormClosing, AddressOf MainFormClosingCleanup
            AddHandler recoveryTimer.Tick, AddressOf SaveRecoveryCopy
            AllowDrop = True
        ...
        Private Sub MainFormShown(sender As Object, e As EventArgs)
            BeginInvoke(New MethodInvoker(AddressOf NormalizeLayout))
            If OfferRecovery() Then Return
            Dim markerFolder As String = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "FlowForgeEducation")
            Dim marker As String = Path.Combine(markerFolder, "welcome.dat")
        ...
        Private ReadOnly Property RecoveryFile As String
            Get
                Return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "FlowForgeEducation", "Recovery.flowapp")
            End Get
        End Property

        Private Sub SaveRecoveryCopy(sender As Object, e As EventArgs)
            If project Is Nothing OrElse project.Forms Is Nothing OrElse project.Forms.Count = 0 Then Return
            Try
                SaveCurrent()
                Directory.CreateDirectory(Path.GetDirectoryName(RecoveryFile))
                Dim temporary As String = RecoveryFile & ".tmp"
                ProjectStorage.Save(temporary, project)
                File.Copy(temporary, RecoveryFile, True)
                File.Delete(temporary)
                status.Text = "Cópia de recuperação atualizada"
            Catch
            End Try
        End Sub

        Private Function OfferRecovery() As Boolean
            If Not File.Exists(RecoveryFile) Then Return False
            Dim recover As Boolean = MessageBox.Show("O FlowForge encontrou uma cópia automática de um projeto que pode não ter sido salvo. Deseja recuperá-la?", "Recuperação de projeto", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes
            If recover Then
                Try
                    project = ProjectStorage.Load(RecoveryFile)
                    currentFile = Nothing
                    activeIndex = 0
                    MaterializeProjectLibrariesForDesigner()
                    RefreshProject()
                    status.Text = "Projeto recuperado — use Salvar como para guardá-lo"
                Catch ex As Exception
                    MessageBox.Show("A cópia de recuperação não pôde ser aberta: " & ex.Message, "Recuperação", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    recover = False
                End Try
            End If
            Try
        ...
        Private Sub MainFormClosingCleanup(sender As Object, e As FormClosingEventArgs)
            ...
            Try
                ...
                If File.Exists(RecoveryFile & ".tmp") Then File.Delete(RecoveryFile & ".tmp")
            Catch
            End Try
        End Sub

        Private Sub ToggleEducationMode()
            beginnerMode = beginnerModeItem.Checked
            ApplyEducationMode()
        End Sub

        Private Sub ApplyEducationMode()
            If beginnerModeItem IsNot Nothing Then beginnerModeItem.Checked = beginnerMode
            For Each menuItem As ToolStripItem In advancedMenus
                menuItem.Visible = Not beginnerMode
            Next
            FilterToolbox(Nothing, EventArgs.Empty)
            If beginnerMode Then
                status.Text = "Modo iniciante: ferramentas essenciais e dicas ativadas"
            Else
                status.Text = "Modo avançado: todos os recursos estão disponíveis"
            End If
        End Sub

        Private Sub ShowEducationCenter()
            Using dialog As New EducationCenterForm()
                dialog.ShowDialog(Me)
            End Using
        End Sub

        Private Sub ShowFirstTutorial()
            Using dialog As New EducationCenterForm("", 0)
                dialog.ShowDialog(Me)
            End Using
        End Sub

        Private Sub ExplainSelectedCode()
            ShowWorkspaceTab(1)
            Dim selected As String = code.SelectedText
            If String.IsNullOrWhiteSpace(selected) Then selected = CurrentMethodText()
            Using dialog As New EducationCenterForm(selected, 6)
                dialog.ShowDialog(Me)
            End Using
        End Sub

        Private Function CurrentMethodText() As String
            Dim position As Integer = code.SelectionStart
            Dim before As String = code.Text.Substring(0, Math.Min(position, code.TextLength))
            Dim startMatch As Match = Regex.Match(before, "(?im)^\s*(Public|Private|Protected|Friend)?\s*(Async\s+)?(Sub|Function)\s+\w+.*$", RegexOptions.RightToLeft)
            If Not startMatch.Success Then Return code.Text
            Dim finish As Match = Regex.Match(code.Text.Substring(startMatch.Index), "(?im)^\s*End\s+(Sub|Function)\s*$")
            If Not finish.Success Then Return code.Text.Substring(startMatch.Index)
            Return code.Text.Substring(startMatch.Index, finish.Index + finish.Length)
        End Function

        Private Sub InsertGuided(kind As String)
            ShowWorkspaceTab(1)
            Dim sample As String
            Select Case kind
                Case "message"
                    sample = "MessageBox.Show(""Olá! Meu programa está funcionando!"")"
                Case "if"
                    sample = "Dim idade As Integer = 10" & vbCrLf & "If idade >= 10 Then" & vbCrLf & "    MessageBox.Show(""Você conseguiu!"")" & vbCrLf & "Else" & vbCrLf & "    MessageBox.Show(""Tente novamente"")" & vbCrLf & "End If"
                Case "for"
                    sample = "For numero As Integer = 1 To 5" & vbCrLf & "    MessageBox.Show(numero.ToString())" & vbCrLf & "Next"
                Case "try"
                    sample = "Try" & vbCrLf & "    ' Coloque aqui o código que pode falhar" & vbCrLf & "Catch ex As Exception" & vbCrLf & "    MessageBox.Show(""Algo deu errado: "" & ex.Message)" & vbCrLf & "End Try"
                Case "form"
                    Dim target As FormData = project.Forms.FirstOrDefault(Function(item) Not item.Name.Equals(CurrentDocument.Name, StringComparison.OrdinalIgnoreCase))
                    If target Is Nothing Then
                        MessageBox.Show("Primeiro adicione um segundo Form no modo avançado.", "Abrir outro Form", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        Return
                    End If
                    sample = target.Name & ".Show()"
                Case "input"
                    sample = "Dim nome As String = Microsoft.VisualBasic.Interaction.InputBox(""Qual é o seu nome?"", ""Pergunta"")" & vbCrLf & "MessageBox.Show(""Olá, "" & nome & ""!"")"
                Case "list"
                    sample = "ListBox1.Items.Add(""Novo item"")" & vbCrLf & "If ListBox1.SelectedItem IsNot Nothing Then" & vbCrLf & "    MessageBox.Show(ListBox1.SelectedItem.ToString())" & vbCrLf & "End If"
                Case "color"
                    sample = "Me.BackColor = Color.CornflowerBlue" & vbCrLf & "Button1.ForeColor = Color.White"
                Case "function"
                    sample = "Private Function Dobro(valor As Integer) As Integer" & vbCrLf & "    Return valor * 2" & vbCrLf & "End Function"
                Case Else
                    Return
            End Select
            If kind = "function" Then
                InsertClassMember(sample)
            Else
        ...
        Private Sub BuildInterface()
            ...
            Dim basicExamples As ToolStripMenuItem = Menu("⭐ Iniciante", _
            ...
            beginnerModeItem = CheckItem("Modo iniciante", True, AddressOf ToggleEducationMode, "help")
            Dim guidedMenu As ToolStripMenuItem = Menu("Inserir código guiado", _
                Item("Mostrar uma mensagem", Sub() InsertGuided("message"), "help"), _
                Item("Tomar uma decisão — If", Sub() InsertGuided("if"), "code"), _
                Item("Repetir — For", Sub() InsertGuided("for"), "redo"), _
                Item("Tentar com segurança — Try", Sub() InsertGuided("try"), "run"), _
                Item("Perguntar um texto — InputBox", Sub() InsertGuided("input"), "help"), _
                Item("Trabalhar com uma lista", Sub() InsertGuided("list"), "code"), _
                Item("Alterar cores", Sub() InsertGuided("color"), "properties"), _
                Item("Criar uma Function", Sub() InsertGuided("function"), "code"), _
                Item("Abrir outro Form", Sub() InsertGuided("form"), "form"))
            Dim starterProjects As ToolStripMenuItem = Menu("Projetos para começar", _
                Item("Olá, mundo", Sub() CreateLearningProject("hello"), "help"), _
                Item("Contador de cliques", Sub() CreateLearningProject("counter"), "run"), _
                Item("Semáforo de cores", Sub() CreateLearningProject("traffic"), "properties"), _
                Item("Quiz simples", Sub() CreateLearningProject("quiz"), "code"), _
                Item("Lista de tarefas", Sub() CreateLearningProject("tasks"), "code"), _
                Item("Adivinhe o número", Sub() CreateLearningProject("guess"), "run"), _
                Item("Aplicativo com dois Forms", Sub() CreateLearningProject("multiform"), "form"))
            Dim educationMenu As ToolStripMenuItem = Menu("Aprender", _
                Item("Central de aprendizado", AddressOf ShowEducationCenter, "help", Keys.F1), _
                Item("Primeiro tutorial", AddressOf ShowFirstTutorial, "run"), _
                Item("Entenda o código selecionado", AddressOf ExplainSelectedCode, "code"), _
                Item("Verificar meu código", AddressOf VerifyMyCode, "build", Keys.Control Or Keys.F7), _
                Item("Mapa do código", AddressOf ShowCodeMap, "code"), _
                Item("Verificar saúde do projeto", AddressOf ShowProjectHealth, "build"), _
                Item("Desafio surpresa", AddressOf ShowRandomChallenge, "help"), _
                Item("Quiz de Visual Basic", Sub() ShowEducationTab(7), "help"), _
                Item("Meu caderno", Sub() ShowEducationTab(8), "code"), _
                Item("Guia de atalhos", Sub() ShowEducationTab(9), "help"), _
                Menu("Acessibilidade do editor", Item("Aumentar fonte", Sub() ChangeEditorZoom(1.0F), "code", Keys.Control Or Keys.Add), Item("Diminuir fonte", Sub() ChangeEditorZoom(-1.0F), "code", Keys.Control Or Keys.Subtract), Item("Restaurar fonte", Sub() ResetEditorZoom(), "undo")), _
                starterProjects, guidedMenu, New ToolStripSeparator(), beginnerModeItem)
            Dim helpMenu As ToolStripMenuItem = Menu("Ajuda", Item("Sobre o FlowForge Education", AddressOf ShowAbout, "help"))
            menuStrip.Items.AddRange(New ToolStripItem() {fileMenu, editMenu, viewMenu, educationMenu, projectMenu, formatMenu, buildMenu, runMenu, toolsMenu, examplesMenu, helpMenu})
            advancedMenus.Add(projectMenu)
            advancedMenus.Add(formatMenu)
            advancedMenus.Add(toolsMenu)
            ...
            educationTools.Items.Add(Button("Explicar", AddressOf ExplainSelectedCode, "code"))
            educationTools.Items.Add(Button("Verificar", AddressOf VerifyMyCode, "build"))

            Dim statusBar As New StatusStrip()
            Dim statusSpacer As New ToolStripStatusLabel() With {.Spring = True}
            educationTip.ForeColor = Color.FromArgb(40, 105, 180)
            statusBar.Items.Add(status) : statusBar.Items.Add(New ToolStripStatusLabel("|")) : statusBar.Items.Add(educationTip) : statusBar.Items.Add(statusSpacer) : statusBar.Items.Add(formStatus) : statusBar.Items.Add(New ToolStripStatusLabel("|")) : statusBar.Items.Add(selectionStatus) : statusBar.Items.Add(New ToolStripStatusLabel("|")) : statusBar.Items.Add(caretStatus)
        ...
        Private Sub UpdateWindowTitle()
            ...
            Text = "FlowForge Education - " & projectName & If(HasDirtyDocuments(), " *", "")
        End Sub
        ...
        Private Sub ShowAbout()
            MessageBox.Show("FlowForge Education 0.10.1 Preview" & vbCrLf & vbCrLf & "Aprenda programação criando aplicativos reais em Visual Basic." & vbCrLf & "Inclui 24 controles FlowForge, editores completos, exemplos, quiz, caderno e progresso." & vbCrLf & ".NET Framework 4.8" & vbCrLf & "Fork educacional independente do FlowForge Studio.", "Sobre o FlowForge Education", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End Sub
        ...
        Private Sub DesignerSelectionChanged(sender As Object, selected As Object)
            ...
            educationTip.Text = ComponentLearningTip(selected)
        ...
        Private Sub NewProject()
            ...
            project = New FlowProject() With {.Name = "MeuProjeto"} : project.Forms.Add(FormData.CreateDefault("Form1", True)) : currentFile = Nothing : activeIndex = 0 : activeClassIndex = -1 : activeModuleIndex = -1 : activeUserControlIndex = -1 : documentTabs.TabPages.Clear() : RefreshProject() : savedProjectState = ProjectState()
        End Sub
        ...
        Private Function ConfirmDiscardChanges() As Boolean
            ...
            Dim answer As DialogResult = MessageBox.Show("O projeto possui alterações não salvas. Deseja salvar antes de continuar?", "FlowForge Education", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning)
        ...
        Private Sub VerifyMyCode()
            SaveCurrent()
            Dim report As String = LearningCodeChecker.Check(code.Text)
            Using dialog As New Form With {.Text = "Verificar meu código", .Width = 650, .Height = 480, .StartPosition = FormStartPosition.CenterParent, .MinimizeBox = False}
                Dim output As New RichTextBox With {.Dock = DockStyle.Fill, .ReadOnly = True, .BackColor = Color.White, .Font = New Font("Segoe UI", 11.0F), .Text = report, .BorderStyle = BorderStyle.None}
                Dim closeButton As New System.Windows.Forms.Button With {.Text = "Voltar ao código", .Dock = DockStyle.Bottom, .Height = 42, .DialogResult = DialogResult.OK}
        ...
        Private Sub ShowCodeMap()
            SaveCurrent()
            ShowLearningReport("Mapa do código", CodeMapBuilder.Build(code.Text))
        End Sub

        Private Sub ShowProjectHealth()
            SaveCurrent()
            ShowLearningReport("Saúde do projeto", ProjectHealthChecker.Check(project))
        End Sub

        Private Sub ShowRandomChallenge()
            ShowEducationTab(3)
        End Sub

        Private Sub ShowEducationTab(tabIndex As Integer)
            Using dialog As New EducationCenterForm("", tabIndex)
                dialog.ShowDialog(Me)
            End Using
        End Sub

        Private Sub ShowLearningReport(title As String, report As String)
            Using dialog As New Form With {.Text = title, .Width = 680, .Height = 520, .StartPosition = FormStartPosition.CenterParent, .MinimizeBox = False}
                Dim output As New RichTextBox With {.Dock = DockStyle.Fill, .ReadOnly = True, .BackColor = Color.White, .Font = New Font("Segoe UI", 11.0F), .Text = report, .BorderStyle = BorderStyle.None}
                Dim closeButton As New System.Windows.Forms.Button With {.Text = "Fechar", .Dock = DockStyle.Bottom, .Height = 42, .DialogResult = DialogResult.OK}
                dialog.Controls.Add(output)
                dialog.Controls.Add(closeButton)
                dialog.AcceptButton = closeButton
                dialog.ShowDialog(Me)
        ...
        Private Sub ChangeEditorZoom(amount As Single)
            Dim newSize As Single = Math.Max(8.0F, Math.Min(28.0F, code.Font.Size + amount))
            code.Font = New Font(code.Font.FontFamily, newSize, code.Font.Style)
            codeRuler.Invalidate()
            status.Text = "Fonte do editor: " & newSize.ToString("0") & " pt"
        End Sub

        Private Sub ResetEditorZoom()
            code.Font = New Font("Consolas", 11.0F)
            codeRuler.Invalidate()
            status.Text = "Fonte do editor restaurada"
        End Sub

        Private Shared Function ComponentLearningTip(selected As Object) As String
            If selected Is Nothing Then Return "Dica: arraste um componente e dê dois cliques para criar sua ação"
            If TypeOf selected Is RoundedButton Then Return "Botão arredondado: use Click, Text, CornerRadius, BackColor e BorderColor"
            If TypeOf selected Is System.Windows.Forms.Button Then Return "Button: use o evento Click e altere Text, Enabled ou BackColor"
            If TypeOf selected Is Label Then Return "Label: mostra texto; altere a propriedade Text pelo código"
            If TypeOf selected Is TextBox Then Return "TextBox: leia Text; use TextChanged para reagir à digitação"
            If TypeOf selected Is PictureBox Then Return "PictureBox: use Image, SizeMode e o evento Click"
            If TypeOf selected Is GradientPanel Then Return "Painel gradiente: combine StartColor, EndColor e GradientAngle"
            If TypeOf selected Is LedIndicator Then Return "LED indicador: altere IsOn e escolha OnColor e OffColor"
            If TypeOf selected Is ToggleSwitch Then Return "Chave: leia Checked e use o evento CheckedChanged"
            If TypeOf selected Is DigitalDisplay Then Return "Display: escolha SevenSegment, DotMatrix ou Text em DisplayStyle; altere Value e Digits"
            If TypeOf selected Is CircularProgress Then Return "Progresso circular: use Value, Maximum, ProgressColor e o evento ValueChanged"
            If TypeOf selected Is LevelMeter Then Return "Medidor de nível: Value acende as barras verdes, amarelas e vermelhas"
            If TypeOf selected Is StarRating Then Return "Estrelas: leia Rating e programe o evento RatingChanged"
            If TypeOf selected Is NumericKnob Then Return "Knob: arraste o mouse para mudar Value e use ValueChanged"
            If TypeOf selected Is BadgeLabel Then Return "Badge: altere Text, BadgeColor e ForeColor"
            If TypeOf selected Is SeparatorLine Then Return "Separador: configure LineColor e Thickness"
            If TypeOf selected Is CardPanel Then Return "Card: painel para agrupar visualmente informações; configure BackColor e BorderColor"
            If TypeOf selected Is BatteryIndicator Then Return "Bateria: Value vai de 0 a 100; Charging mostra o símbolo de carga"
            If TypeOf selected Is SignalStrength Then Return "Sinal: altere Value de 0 a 100 para acender as barras"
            If TypeOf selected Is ThermometerGauge Then Return "Termômetro: use Value, Minimum, Maximum e ValueChanged"
            If TypeOf selected Is AnalogGauge Then Return "Medidor analógico: configure Value, Maximum e Caption"
            If TypeOf selected Is LoadingSpinner Then Return "Carregamento: use Active para iniciar ou parar a animação"
            If TypeOf selected Is NotificationBanner Then Return "Notificação: escolha Information, Success, Warning ou Error em BannerStyle"
            If TypeOf selected Is ToggleButton Then Return "Botão alternável: leia Checked e programe CheckedChanged"
            If TypeOf selected Is ColorSwatch Then Return "Cor: dê dois cliques ou chame ChooseColor; leia SelectedColor"
            If TypeOf selected Is NavigationButton Then Return "Navegação: escolha Left, Right, Up ou Down em Direction e programe Click"
            If TypeOf selected Is MarqueeLabel Then Return "Texto móvel: altere Text e Speed para criar letreiros"
            If TypeOf selected Is RichTextEditor Then Return "Editor rico: use Text ou Rtf; a barra interna oferece fontes, cores, alinhamento, abrir e salvar"
            If TypeOf selected Is SyntaxCodeEditor Then Return "Editor de código: escolha Language, use Text, LineCount, GoToLine e CodeChanged"
            If TypeOf selected Is ListBox Then Return "ListBox: use Items.Add, SelectedItem e SelectedIndexChanged"
            If TypeOf selected Is ComboBox Then Return "ComboBox: use Items, SelectedItem e SelectedIndexChanged"
            If TypeOf selected Is System.Windows.Forms.Timer Then Return "Timer: defina Interval, Enabled e programe o evento Tick"
            If TypeOf selected Is Form Then Return "Form: esta é a janela; configure Text, Size e StartPosition"
            Return selected.GetType().Name & ": veja Name e as propriedades; dê dois cliques para criar o evento principal"
        End Function

        Private Sub CreateLearningProject(kind As String)
            If Not ConfirmDiscardChanges() Then Return
            If MessageBox.Show("Criar este projeto guiado agora?", "Projeto para começar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) <> DialogResult.Yes Then Return
            Dim document As FormData = FormData.CreateDefault("Form1", True)
            document.Properties("ClientSize") = "650, 400"
            Dim projectName As String
            Dim additionalForm As FormData = Nothing
            Select Case kind
                Case "hello"
                    projectName = "OlaMundo"
                    document.Properties("Text") = "Meu primeiro aplicativo"
                    document.Controls.Add(LearningControl("Label", "lblTitulo", "Meu primeiro programa", 165, 70, 320, 45))
                    document.Controls.Add(LearningControl("Button", "btnOla", "Clique em mim!", 225, 155, 200, 55))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private Sub btnOla_Click() Handles btnOla.Click" & vbCrLf & "        MessageBox.Show(""Olá! Você criou um aplicativo!"")" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case "counter"
                    projectName = "ContadorDeCliques"
                    document.Properties("Text") = "Contador de cliques"
                    document.Controls.Add(LearningControl("Label", "lblContador", "0", 270, 70, 110, 70))
                    document.Controls.Add(LearningControl("Button", "btnContar", "Somar um", 225, 175, 200, 55))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private contador As Integer = 0" & vbCrLf & vbCrLf & "    Private Sub btnContar_Click() Handles btnContar.Click" & vbCrLf & "        contador = contador + 1" & vbCrLf & "        lblContador.Text = contador.ToString()" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case "traffic"
                    projectName = "SemaforoDeCores"
                    document.Properties("Text") = "Semáforo de cores"
                    document.Controls.Add(LearningControl("Panel", "painelCor", "", 200, 45, 250, 190))
                    document.Controls.Add(LearningControl("Button", "btnVermelho", "Vermelho", 75, 285, 150, 45))
                    document.Controls.Add(LearningControl("Button", "btnAmarelo", "Amarelo", 250, 285, 150, 45))
                    document.Controls.Add(LearningControl("Button", "btnVerde", "Verde", 425, 285, 150, 45))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private Sub btnVermelho_Click() Handles btnVermelho.Click" & vbCrLf & "        painelCor.BackColor = Color.Red" & vbCrLf & "    End Sub" & vbCrLf & vbCrLf & "    Private Sub btnAmarelo_Click() Handles btnAmarelo.Click" & vbCrLf & "        painelCor.BackColor = Color.Gold" & vbCrLf & "    End Sub" & vbCrLf & vbCrLf & "    Private Sub btnVerde_Click() Handles btnVerde.Click" & vbCrLf & "        painelCor.BackColor = Color.LimeGreen" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case "tasks"
                    projectName = "MinhaListaDeTarefas"
                    document.Properties("Text") = "Minha lista de tarefas"
                    document.Controls.Add(LearningControl("TextBox", "txtTarefa", "", 70, 55, 390, 35))
                    document.Controls.Add(LearningControl("Button", "btnAdicionar", "Adicionar", 475, 52, 110, 42))
                    document.Controls.Add(LearningControl("ListBox", "listaTarefas", "", 70, 115, 515, 180))
                    document.Controls.Add(LearningControl("Button", "btnRemover", "Remover selecionada", 205, 315, 240, 45))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private Sub btnAdicionar_Click() Handles btnAdicionar.Click" & vbCrLf & "        If txtTarefa.Text.Trim() = """" Then Return" & vbCrLf & "        listaTarefas.Items.Add(txtTarefa.Text.Trim())" & vbCrLf & "        txtTarefa.Clear()" & vbCrLf & "        txtTarefa.Focus()" & vbCrLf & "    End Sub" & vbCrLf & vbCrLf & "    Private Sub btnRemover_Click() Handles btnRemover.Click" & vbCrLf & "        If listaTarefas.SelectedIndex >= 0 Then" & vbCrLf & "            listaTarefas.Items.RemoveAt(listaTarefas.SelectedIndex)" & vbCrLf & "        End If" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case "guess"
                    projectName = "AdivinheONumero"
                    document.Properties("Text") = "Adivinhe o número"
                    document.Controls.Add(LearningControl("Label", "lblDica", "Digite um número de 1 a 20", 145, 65, 360, 45))
                    document.Controls.Add(LearningControl("TextBox", "txtPalpite", "", 225, 135, 200, 35))
                    document.Controls.Add(LearningControl("Button", "btnTentar", "Tentar", 225, 195, 200, 50))
                    document.Controls.Add(LearningControl("Label", "lblTentativas", "Tentativas: 0", 225, 275, 200, 35))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private ReadOnly sorteio As New Random()" & vbCrLf & "    Private numeroSecreto As Integer" & vbCrLf & "    Private tentativas As Integer" & vbCrLf & vbCrLf & "    Private Sub Form1_Load() Handles MyBase.Load" & vbCrLf & "        numeroSecreto = sorteio.Next(1, 21)" & vbCrLf & "    End Sub" & vbCrLf & vbCrLf & "    Private Sub btnTentar_Click() Handles btnTentar.Click" & vbCrLf & "        Dim palpite As Integer" & vbCrLf & "        If Not Integer.TryParse(txtPalpite.Text, palpite) Then" & vbCrLf & "            lblDica.Text = ""Digite somente números.""" & vbCrLf & "            Return" & vbCrLf & "        End If" & vbCrLf & "        tentativas += 1" & vbCrLf & "        lblTentativas.Text = ""Tentativas: "" & tentativas.ToString()" & vbCrLf & "        If palpite = numeroSecreto Then" & vbCrLf & "            lblDica.Text = ""Parabéns! Você acertou!""" & vbCrLf & "        ElseIf palpite < numeroSecreto Then" & vbCrLf & "            lblDica.Text = ""Tente um número maior.""" & vbCrLf & "        Else" & vbCrLf & "            lblDica.Text = ""Tente um número menor.""" & vbCrLf & "        End If" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case "multiform"
                    projectName = "DuasJanelas"
                    document.Properties("Text") = "Janela principal"
                    document.Controls.Add(LearningControl("Label", "lblInstrucao", "Clique para conhecer a segunda janela", 120, 85, 410, 45))
                    document.Controls.Add(LearningControl("Button", "btnAbrir", "Abrir Form2", 225, 165, 200, 55))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private Sub btnAbrir_Click() Handles btnAbrir.Click" & vbCrLf & "        Form2.ShowDialog()" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                    additionalForm = FormData.CreateDefault("Form2", False)
                    additionalForm.Properties("Text") = "Segunda janela"
                    additionalForm.Properties("ClientSize") = "500, 300"
                    additionalForm.Controls.Add(LearningControl("Label", "lblMensagem", "Você abriu outro Form!", 105, 65, 290, 45))
                    additionalForm.Controls.Add(LearningControl("Button", "btnVoltar", "Voltar", 150, 145, 200, 50))
                    additionalForm.Code = LearningImports() & "Partial Public Class Form2" & vbCrLf & "    Private Sub btnVoltar_Click() Handles btnVoltar.Click" & vbCrLf & "        Me.Close()" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
                Case Else
                    projectName = "QuizSimples"
                    document.Properties("Text") = "Quiz simples"
                    document.Controls.Add(LearningControl("Label", "lblPergunta", "Qual é a capital do Brasil?", 120, 65, 410, 45))
                    document.Controls.Add(LearningControl("TextBox", "txtResposta", "", 150, 135, 350, 35))
                    document.Controls.Add(LearningControl("Button", "btnResponder", "Responder", 225, 205, 200, 50))
                    document.Controls.Add(LearningControl("Label", "lblResultado", "Digite sua resposta", 150, 285, 350, 35))
                    document.Code = LearningImports() & "Partial Public Class Form1" & vbCrLf & "    Private Sub btnResponder_Click() Handles btnResponder.Click" & vbCrLf & "        If String.Equals(txtResposta.Text.Trim(), ""Brasília"", StringComparison.OrdinalIgnoreCase) Then" & vbCrLf & "            lblResultado.Text = ""Parabéns, resposta correta!""" & vbCrLf & "            lblResultado.ForeColor = Color.Green" & vbCrLf & "        Else" & vbCrLf & "            lblResultado.Text = ""Tente novamente.""" & vbCrLf & "            lblResultado.ForeColor = Color.Firebrick" & vbCrLf & "        End If" & vbCrLf & "    End Sub" & vbCrLf & "End Class"
            End Select
            project = New FlowProject With {.Name = projectName}
            project.Forms.Add(document)
            If additionalForm IsNot Nothing Then project.Forms.Add(additionalForm)
            currentFile = Nothing
            activeIndex = 0
            RefreshProject()
            status.Text = "Projeto guiado criado: " & projectName
        End Sub

        Private Shared Function LearningImports() As String
            Return "Imports System" & vbCrLf & "Imports System.Drawing" & vbCrLf & "Imports System.Windows.Forms" & vbCrLf & vbCrLf
        End Function

        Private Shared Function LearningControl(typeName As String, name As String, textValue As String, x As Integer, y As Integer, width As Integer, height As Integer) As ControlData
            Dim item As New ControlData With {.TypeName = typeName, .Name = name}
            item.Properties("Name") = name
            item.Properties("Text") = textValue
            item.Properties("Location") = x.ToString() & ", " & y.ToString()
            item.Properties("Size") = width.ToString() & ", " & height.ToString()
            item.Properties("Font") = "Segoe UI, 12pt"
            Return item
        End Function
        ...
        Private Sub InsertClassMember(memberCode As String)
            Dim insertAt As Integer = code.Text.LastIndexOf("End Class", StringComparison.OrdinalIgnoreCase)
            If insertAt < 0 Then
                MessageBox.Show("Não foi possível localizar 'End Class'. Corrija a estrutura antes de inserir a Function.", "Código VB.NET", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return
            End If
            Dim block As String = "    " & memberCode.Replace(vbCrLf, vbCrLf & "    ") & vbCrLf & vbCrLf
            code.Select(insertAt, 0)
            code.SelectedText = block
            code.Focus()
        End Sub
        ...
        Private Sub CompilerIssueActivated(sender As Object, e As CompilerIssueEventArgs)
            ...
            If index >= 0 Then OpenFormCode(index) : SelectEditorLocation(e.LineNumber, e.ColumnNumber) : Return
            If project.Classes IsNot Nothing Then
        ...
        Private Sub SelectEditorLocation(lineNumber As Integer, columnNumber As Integer)
            ...
            code.ScrollToCaret()
            code.Focus()
            status.Text = "Erro — linha " & lineNumber & ", coluna " & columnNumber
        End Sub
        Private Sub SaveAll()
            SaveProject()
        ...
        Private Sub SaveProject()
            ...
            SaveCurrent() : ProjectStorage.Save(currentFile, project) : savedProjectState = ProjectState() : ClearDirtyIndicators() : RecentProjectStore.Add("FlowForgeEducation", currentFile) : RefreshRecentProjectsMenu() : status.Text = "Salvo: " & Path.GetFileName(currentFile)
        End Sub
        ...
        Private Sub LoadProjectFromPath(filePath As String)
            ...
            Try
                ...
                RecentProjectStore.Add("FlowForgeEducation", filePath)
                RefreshRecentProjectsMenu()
            Catch ex As Exception
        ...
        Private Sub RefreshRecentProjectsMenu()
            ...
            Dim files As List(Of String) = RecentProjectStore.Load("FlowForgeEducation")
        ...
        Private Sub ClearRecentProjects()
            RecentProjectStore.Clear("FlowForgeEducation")
            RefreshRecentProjectsMenu()
        End Sub
        ...
        Private Shared Function Item(text As String, action As Action, Optional iconName As String = "", Optional shortcut As Keys = Keys.None) As ToolStripItem
            ...
            Return m
        End Function
        Private Shared Function CheckItem(text As String, initialValue As Boolean, action As Action, iconName As String) As ToolStripMenuItem
        ...
        Private Sub SaveToolStripLayout()
            Try
                ...
                SaveOneToolStripLayout(lines, educationTools)
        ...
        Private Sub RestoreToolStripLayout(shell As ToolStripContainer)
            Try
                ...
                AddToolbarToMap(bars, educationTools)
